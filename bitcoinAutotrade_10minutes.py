...
def get_target_price_minute10(ticker, k):
    """변동성 돌파 전략으로 매수 목표가 조회"""
...
def get_start_time_minute10(ticker):
    """시작 시간 조회"""
...
while True:
    try:
        ...
        start_time = get_start_time_minute10("KRW-IQ") #9:00
        end_time = start_time + datetime.timedelta(minutes=10) #9:00 + 10분

        # 9:00 < 현재 < # 9:09:55
        if start_time < now < end_time - datetime.timedelta(seconds=5):
            target_price = get_target_price_minute10("KRW-IQ", 0.5)
